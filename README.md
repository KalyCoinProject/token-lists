...
The JSON schema represents the technical specification for a token list which can be used in a dApp interface, such as the KalySwap Interface.

## What are token lists?

KalySwap Token Lists is a specification for lists of token metadata (e.g. address, decimals, ...) that can be used by any dApp interfaces that needs one or more lists of tokens.
...
Token list JSON must validate against the [JSON schema](https://json-schema.org/) in order to be used in the KalySwap Interface.
Tokens on token lists, and token lists themselves, are tagged so that users can easily find tokens.

## JSON Schema $id

The JSON schema ID is [tokenlist.schema.json](https://github.com/kalycoinproject/token-lists/tokenlist.schema.json)
...
```typescript
import { TokenList, schema } from '@kalycoinproject/token-lists'
